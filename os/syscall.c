					...
					const int request[NTHREAD][LOCK_POOL_SIZE])
{
	...
	// don't look at threads not in use
	struct proc *p = curr_proc();
	for (int t = 0; t < NTHREAD; t++)
		finish[t] = (p->threads[t].state == T_UNUSED ||
					 p->threads[t].state == EXITED) ? 1 : 0;

	// find a satisfiable request
	int progress = 1;
	while (progress) {
		progress = 0;
		for (int t = 0; t < NTHREAD; t++) {
			...
			int can_finish = 1;
			for (int i = 0; i < LOCK_POOL_SIZE; i++) { // check all requests for the thread
				if (request[t][i] > work[i]) {
					can_finish = 0;
					break;
				}
			}
			if (can_finish) {
				// simulate finishing and releasing locks
				for (int i = 0; i < LOCK_POOL_SIZE; i++) work[i] += allocation[t][i];
				finish[t] = 1;
				progress = 1;
			}
		}
	...
	for (int t = 0; t < NTHREAD; t++) { // if any thread isn't finished, there is a deadlock
		if (!finish[t]) return 1; // found it
	}
...
int sys_mutex_create(int blocking)
{
	...
	curr_proc()->mutex_available[mutex_id] = 1; // mutex starts unlocked
...
int sys_mutex_lock(int mutex_id)
{
	...
	if (p->deadlock_detect_enabled) {
		// this thread will request the mutex
	...
	if (p->deadlock_detect_enabled) {
		// lock obtained, move from request to allocation
		p->mutex_request[tid][mutex_id] = 0;
...
int sys_semaphore_create(int res_count)
{
	...
	curr_proc()->sem_available[sem_id] = res_count; // initialize available count
...
int sys_semaphore_up(int semaphore_id)
{
	...
	if (p->deadlock_detect_enabled) {
		// thread will release one unit. decrement allocation, increment available
...
int sys_semaphore_down(int semaphore_id)
{
	...
	if (p->deadlock_detect_enabled) {
		// this thread will request one unit
	...
	if (p->deadlock_detect_enabled) {
		// unit aquired, move from request to allocation
		p->sem_request[tid][semaphore_id] = 0;
		p->sem_allocation[tid][semaphore_id]++;
		p->sem_available[semaphore_id]--;
	}
