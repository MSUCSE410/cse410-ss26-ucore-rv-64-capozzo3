...
struct inode *ialloc(uint dev, short type)
{
	...
	for (inum = 1; inum < sb.ninodes; inum++) {
		...
		if (dip->type == 0) { // a free inode
			...
			dip->type = type;
			dip->nlink = 1; // initialize nlink to 1
			bwrite(bp);
			brelse(bp);
...
void iupdate(struct inode *ip)
{
	...
	dip->nlink = ip->nlink; // sync to disk
...
int dirunlink(struct inode *dp, char *name)
{
	...
	// zero out dirent entry
	memset(&de, 0, sizeof(de));
	if (writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de)) panic("dirunlink");
	return 0;
}
...
struct inode *namei(char *path)
{
	...
	if (dp == 0)
		panic("fs dumped.\n");
	struct inode *result = dirlookup(dp, path + skip, 0);
	iput(dp); // release after dirlookup is done with dp
	return result;
}
